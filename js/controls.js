...
export function initControls(player, loadItem, playlist, getCurrentIndex) {
  ...
  controlsWrap.innerHTML = `
    <div class="hover-tip-layer"></div>
    <div class="progress-row">
      <div class="time-label" id="time-start">00:00</div>
      ...
      <div class="time-label" id="time-end">00:00</div>
    </div>
    <div class="bottom-controls">
      ...
      <div class="right-controls">
        <button class="btn" id="btn-full"></button>
      </div>
    </div>
  `;
  ...
  player.on('play', () => btnPlay.innerHTML = icons.pause);
  player.on('pause', () => btnPlay.innerHTML = icons.play);

  // Prev/Next
  btnPrev.addEventListener('click', () => loadItem(getCurrentIndex() - 1));
  btnNext.addEventListener('click', () => loadItem(getCurrentIndex() + 1));
  ...
  vol.addEventListener('input', (e) => {
    ...
    if (v === 0) player.muted(true); else { player.muted(false); player.volume(v); lastVolumeBeforeMute = v; }
    syncVolumeUI();
  });

  btnMute.addEventListener('click', () => {
    if (player.muted() || player.volume() === 0) { player.muted(false); player.volume(lastVolumeBeforeMute); } 
    else { lastVolumeBeforeMute = player.volume(); player.muted(true); }
    syncVolumeUI();
  });

  // Fullscreen
  btnFull.addEventListener('click', () => player.isFullscreen() ? player.exitFullscreen() : player.requestFullscreen());
  ...
  ['timeupdate','durationchange','loadedmetadata','progress'].forEach(ev => player.on(ev, updateTimes));

  // Seek
  let seeking = false;
  const seekToPct = (p) => { const d = player.duration() || 0; player.currentTime(p * d); };
  const onSeekStart = (e) => { seeking = true; seekToPct(getProgressPctFromEvent(e, progress)); };
  const onSeekMove = (e) => { if(seeking) seekToPct(getProgressPctFromEvent(e, progress)); };
  const onSeekEnd = () => { seeking = false; };

  progress.addEventListener('mousedown', onSeekStart);
  progress.addEventListener('mousemove', onSeekMove);
  // Use capturing to ensure this fires even if the mouse is outside the window
  ...
  progress.addEventListener('mouseleave', () => hoverTip.style.display = 'none');
  ...
  function showControls() {
    ...
    hideTimer = setTimeout(() => { if(!seeking && !player.paused()){ controlsWrap.classList.remove('controls-visible'); controlsWrap.classList.add('controls-hidden'); }},2000);
  }
  ['mousemove','touchstart','keydown'].forEach(evt => container.addEventListener(evt, showControls, {passive:true}));
  player.on('play', showControls); player.on('pause', showControls);

  // Initialize volume & UI
  player.on('volumechange', syncVolumeUI);
  player.ready(() => { player.volume(0.7); player.muted(false); syncVolumeUI(); updateTimes(); });
}
